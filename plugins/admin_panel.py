...
async def addchh(app, call):
  ...
  if ask.text:
    ...
    return
@app.on_callback_query(filters.regex('^leavefrom$'))
async def leavefrom(app, call):
  admins = db.get('admin_list')
  ask = await app.ask(
    call.from_user.id,
    'ارسل معرف لقناة الان !!'
  )
  if ask.text:
    ch = ask.text.replace("https://t.me/","").replace("@","")
    ses = db.get("sessions")
    y = 0
    if not ses:
        return
    ...
    for ix in ses:
        try:
            o = await leave(session=ix, channel=ch)
            if str(o) == "v": continue
        except:
            continue
        y +=1
    await ask.reply(f"تمت عملية الخروج بنجاح!!\nتم الخروج من : {y} حساب .")
    return
...
async def clear(app,call):
  ...
  for i in range(len(ses)):
    try:
      c = temp(name=ses[i][:14], api_id=3895828, api_hash='5229902a14b2512c35688aa152bd9f29', session_string=ses[i], no_updates=True)
    except:
      continue
    try:
      await c.start()
