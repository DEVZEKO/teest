...
async def spam_r(app, query):
    ...
    await app.delete_messages(user_id, [query.message.id])
    chats = db.get('force')
...
- @{i}
    ...
    if int(x) <1:
        ...
        if tex.text:
            ...
            u = db.get("orders") if db.get("orders") else []
            import random; ri = random.randint(100000, 999999)
            if not u:
                db.set("orders", [ri])
            u.append(ri)
            db.set("orders", u)
            await tex.reply(f"🎉 تم انتهاء الطلب بنجاح !! \n⎯ ⎯ ⎯ ⎯ ⎯ ⎯ ⎯ ⎯\nالطلب : {count} .\nالمُرسل : {y} .\n⎯ ⎯ ⎯ ⎯ ⎯ ⎯ ⎯ ⎯\nيمكنك إرسال طلب آخر من هذا النوع .")
            return
...
- @{i}
    ...
    if user_info['coins'] < int(x):
        ...
        if ask2.text and "t.me" in ask2.text:
            ...
            u.append(ri)
            db.set("orders", u)
            await ask2.reply(f"🎉 تم انتهاء الطلب بنجاح !! \n⎯ ⎯ ⎯ ⎯ ⎯ ⎯ ⎯ ⎯\nالطلب : {count} .\nالمُرسل : {y} .\n⎯ ⎯ ⎯ ⎯ ⎯ ⎯ ⎯ ⎯\nيمكنك إرسال طلب آخر من هذا النوع .")
            return
...
- @{i}
    ...
    if ask.text:
        ...
        if int(x) > int(user_info['coins']):
            await ask1.reply(f"مامعك سعر الريأكشنات تحتاج: {x} USD .")
            return
        if count <1:
        ...
        u.append(ri)
        db.set("orders", u)
        await ask1.reply(f"🎉 تم انتهاء الطلب بنجاح !! \n⎯ ⎯ ⎯ ⎯ ⎯ ⎯ ⎯ ⎯\nالطلب : {count} .\nالمُرسل : {y} .\n⎯ ⎯ ⎯ ⎯ ⎯ ⎯ ⎯ ⎯\nيمكنك إرسال طلب آخر من هذا النوع .")
        return
